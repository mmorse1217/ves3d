function err = GenerateBmark_singleVes_Exp(precision)
  ...
  try
    ...
    saveThis(VES3D_PATH, 'x0.txt', X, precision);
    ...
    vel = G*Fb; 
    saveThis(VES3D_PATH, 'SFb.txt', vel, precision);
    vel = vel + v_inf;
    ...
    saveThis(VES3D_PATH, 'tension.txt', sig, precision);
    vel = vel + Fs(sig);                                                    

    %% New position
    Xn = X + ts*vel; 
    saveThis(VES3D_PATH, 'xnew.txt', Xn, precision);
