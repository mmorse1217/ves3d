...
// prototype for BLAS single-precision matrix-matrix multiply
//#define sgemm sgemm
...
// constructor
blas_sht::blas_sht(int pp, int nnum_vesicles, char * legTransFname, char* legTransInvFname, char* d1legTransFname, char* d2legTransFname)
{
  p=pp;
...
void blas_sht::forward(scalar * inputs, scalar * outputs)
{
  ...
  scalar * dft_outputs = &temp_data[0];
  // actual computation:
  {
    int l=dft_size;
    int n=num_dft_inputs;
    int m=dft_size;
    scalar alpha = 1;
    scalar beta = 0;
    int lda = dft_size;
    ...
    sgemm("N", "N", &l, &n, &m, &alpha, &dft_forward[0], &lda, inputs, &ldb, &beta, dft_outputs, &ldc);
    s = get_seconds() -s ;
    cout<<"CPU BLAS Forward DFT time: "<<s<<endl;
  }
 ...
 //  ----- rearrange data for Legendre transform -----
  ...
  // actual computation
  {
    ...
    int ldb = p+1;
    int ldc = p+1;
    sgemm("N", "N", &l, &n, &m, &alpha, &legendre_matrices[leg_matrix_pointer], &lda, legendre_inputs+leg_input_pointer, &ldb, &beta, legendre_outputs+leg_outputs_pointer, &ldc);
  }
...
void blas_sht::backward(scalar * inputs, scalar * outputs)
{
  // p is the "order"; we have 2*p points on each "parallel" and p+1 points on each meridian
...
//    for (int i=0; i<2*p*2*p; i++)
//      cout<<dft_matrix[i]<<endl;
  ...
  // actual computation:
  {
    ...
    int ldb = dft_size;
    int ldc = dft_size;
    sgemm("T", "N", &l, &n, &m, &alpha, dft_matrix, &lda, dft_inputs, &ldb, &beta, dft_outputs, &ldc);
  }
};

//
//void blas_sht::backward_du(scalar * inputs, scalar * outputs)
//{
//  // p is the "order"; we have 2*p points on each "parallel" and p+1 points on each meridian
//
//  // we first do backward Legendre transform, then backward DFT
...
//};
//
//void blas_sht::backward_d2u(scalar * inputs, scalar * outputs)
//{
//  // p is the "order"; we have 2*p points on each "parallel" and p+1 points on each meridian
//
//  // we first do backward Legendre transform, then backward DFT
...
//};
//
//void blas_sht::backward_dv(scalar * inputs, scalar * outputs)
//{
//  // p is the "order"; we have 2*p points on each "parallel" and p+1 points on each meridian
//
//  // we first do backward Legendre transform, then backward DFT
...
//};
//
//void blas_sht::backward_d2v(scalar * inputs, scalar * outputs)
//{
//  // p is the "order"; we have 2*p points on each "parallel" and p+1 points on each meridian
//
//  // we first do backward Legendre transform, then backward DFT
...
//};
//
//void blas_sht::backward_duv(scalar * inputs, scalar * outputs)
//{
//  // p is the "order"; we have 2*p points on each "parallel" and p+1 points on each meridian
//
//  // we first do backward Legendre transform, then backward DFT
