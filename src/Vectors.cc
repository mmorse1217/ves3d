    ...
    const Vectors<T>& y_in, Scalars<T>& xDy_out)
{
    ...
    x_in.device_.DotProduct(x_in.data_, y_in.data_, x_in.GetFunLength(), x_in.n_vecs_, xDy.data_);
}
    ...
    const Vectors<T>& y_in, Vectors<T>& xCy_out)
{
    ...
    x_in.device_.CrossProduct(x_in.data_, y_in.data_, x_in.GetFunLength(), x_in.n_vecs_, xCy.data_);
}
...
{
    assert(x_in.GetDataLength() == 3*v_in.GetDataLength());
    ...
    x_in.device_.axpw(x_in.data_, v_in.data_, w_in.data_, x_in.GetFunLength(), x_in.n_vecs_, xvpw.data_);
}
    ...
    T w_in, Vectors<T>& xvpb_out)
{
    ...
    x_in.device_.axpb(x_in.data_, v_in.data_, w_in, x_in.GetFunLength(), x_in.n_vecs_, xvpb.data_);
}
