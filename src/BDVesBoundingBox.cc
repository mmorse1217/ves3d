...
BDVesBoundingBox<Real_t>::BDVesBoundingBox(Real_t box_size, MPI_Comm c){
        ...
        const Real_t *pos_bd, const int num_points_per_patch_1d, const int num_patches)
{
    ...
    int N_ves = ves_coord_s_up.Dim()/ves_stride/COORD_DIM;
    N_bbox_ = N_ves + num_patches;
    ...
    for(size_t tid=0; tid<omp_p_; tid++){
        size_t a = ((tid+0)*N_ves)/omp_p_;
        size_t b = ((tid+1)*N_ves)/omp_p_;
    ...
    for(size_t tid=0; tid<omp_p_; tid++){
        ...
        for(size_t i=a; i<b; i++){
            Real_t *mini = &BB_min_[i*COORD_DIM + N_ves*COORD_DIM];
            Real_t *maxi = &BB_max_[i*COORD_DIM + N_ves*COORD_DIM];

            Real_t min_tmp[3];
            Real_t max_tmp[3];
            Real_t *first_point = &pos_bd[i*(num_points_per_patch_1d*num_points_per_patch_1d)];
            for(size_t k=0; k<COORD_DIM; k++)
            {
            ...
            for(int i1d=0; i1d<num_points_per_patch_1d; i1d++)
            {
                for(int j1d=0; j1d<num_points_per_patch_1d; j1d++)
                {
                    ...
                    for(size_t k=0; k<COORD_DIM; k++)
                    {
                        min_tmp[k] = std::min(first_point[point_id*COORD_DIM+k], min_tmp[k]);
                        max_tmp[k] = std::max(first_point[point_id*COORD_DIM+k], max_tmp[k]);
                    }
                }
            }
            for(size_t k=0; k<COORD_DIM; k++)
            {
                mini[k] = min_tmp[k] - min_sep/2 - 1e-8;
                maxi[k] = max_tmp[k] + min_sep/2 + 1e-8;
            }
        }
    }
        ...
        const Real_t *pos_bd, const int num_points_per_patch_1d, const int num_patches)
{
    ...
    SetBDVesBoundingBox(ves_coord_s_pvfmm, ves_coord_e_pvfmm, min_sep, sh_order, sh_order_up, 
            pos_bd, num_points_per_patch_1d, num_patches);
}

template<typename Real_t>
void BDVesBoundingBox<Real_t>::SetBDVesBoundingBox(const PVFMMVec_t& BB_min, const PVFMMVec_t& BB_max)
{
...
template<typename Real_t>
void BDVesBoundingBox<Real_t>::GetContactBoundingBoxPair(std::vector< std::pair<size_t, size_t> > &BBIPairs)
{
...
template<typename Real_t>
void BDVesBoundingBox<Real_t>::SetTreeParams()
{
...
template<typename Real_t>
void BDVesBoundingBox<Real_t>::ConstructLocalTree(TREEGRID &BB_let)
{
...
template<typename Real_t>
void BDVesBoundingBox<Real_t>::FindNearPair(TREEGRID &BB_let, std::vector< std::pair<size_t, size_t> > &BBIPairs)
{
    pvfmm::Profile::Tic("NearPair",&comm,true);
    PVFMMVec_t &box_min = BB_let.box_min;
    PVFMMVec_t &box_max = BB_let.box_max;
...
template<typename Real_t>
void BDVesBoundingBox<Real_t>::GlobalBoundingBox(Real_t *scale_xr, Real_t *shift_xr)
{
...
template<typename Real_t>
void BDVesBoundingBox<Real_t>::GenerateBBPoints()
{
...
template<typename Real_t>
bool BDVesBoundingBox<Real_t>::CheckBBCollision(const Real_t *minA, const Real_t *maxA, const Real_t *minB, const Real_t *maxB)
{
    if(box_size_<=0)
    {
