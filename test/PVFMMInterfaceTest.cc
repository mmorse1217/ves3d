...
void nbody(std::vector<Real_t>&  src_coord, std::vector<Real_t>&  src_value,
...
int main(int argc, char** argv){
  ...
  std::vector<Real_t> force(N*DIM);
  std::vector<Real_t> fmm_v(N*DIM);
  ...
  void* ctx=NULL;
  PVFMMEval(&coord[0], &force[0], N, &fmm_v[0], &ctx);
  PVFMMEval(&coord[0], &force[0], N, &fmm_v[0], &ctx);

  {// Check error
    const pvfmm::Kernel<Real_t>& kernel_fn=pvfmm::ker_stokes_vel;
    ...
    { // Sample target points for verifications.
      ...
      for(size_t i=0;i<n_trg;i=i+n_skip){
        ...
        for(size_t j=0;j<kernel_fn.ker_dim[1];j++)
          trg_sample_value.push_back(fmm_v[i*kernel_fn.ker_dim[1]+j]);
        n_trg_sample++;
      }
    }

    // Direct n-body
    std::vector<Real_t> trg_sample_value_(n_trg_sample*kernel_fn.ker_dim[1]);
    nbody(           coord,            force ,
