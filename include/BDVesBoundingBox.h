#ifndef _BDVES_BOUNDING_BOX_H_
#define _BDVES_BOUNDING_BOX_H_
//#define BDVES_BOUNDING_BOX_DEBUG
...
class BDVesBoundingBox{
    public:
        ...
        BDVesBoundingBox(Real_t box_size=-1, MPI_Comm c=MPI_COMM_WORLD);

        // set bounding boxes for vesicles, the bounding boxes are space-time bounding box with minimum separation distance
        void SetBDVesBoundingBox(const PVFMMVec_t& ves_coord_s, const PVFMMVec_t& ves_coord_e, 
        ...
        void SetBDVesBoundingBox(const Vec& ves_coord_s, const Vec& ves_coord_e, 
                const Real_t min_sep, const int sh_order, const int sh_order_up,
                const Real_t *pos_bd, const int num_points_per_patch_1d, const int num_patches);
        // set bounding boxes provided with two points for each bounding box: BB_min, BB_max
        void SetBDVesBoundingBox(const PVFMMVec_t& BB_min, const PVFMMVec_t& BB_max);
    ...
    private:
        ...
        //BDVesBoundingBox(const VesBoundingBox &);
        //BDVesBoundingBox& operator=(const VesBoundingBox &);
...
#include "BDVesBoundingBox.cc"

#endif // _BDVES_BOUNDING_BOX_H_
