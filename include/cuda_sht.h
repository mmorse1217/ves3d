...
// longtitudes).
    ...
    // constructor allocates temp. memory on host, loads
    // Leg. transforms, calls cublasInit(), etc.
    cuda_sht(int p, int num_vesicles, char * legTransFname, char* legTransInvFname, char* d1legTransFname, char* d2legTransFname);
    // destructor just calls cublasShutdown() (memory deallocated
    // aoutomatically)
    ~cuda_sht();
  private:
    int p;
